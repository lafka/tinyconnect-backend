    ...
    serial.list(function(err, ports) {
      if (err) {
        ...
        let ret = _.reduce(ports, function(acc, port) {
          let client = _.find(this._clients, function(client) { return client.usesPort(port) })
          ...
          if (client) {
            ...
            acc[client._ref] = client
            console.log('debug: added port ' + client._ref + ' @' + port.comName)
            haveChanges = true
          }
