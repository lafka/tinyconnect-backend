...
export default class WebSockets {
  ...
  constructor(opts, managerOpts, frontend, backend) {
    this._manager = undefined
    this._api = undefined

    this._connections = []
    this._frontendReleases = []
    this._backendRelease = []
    ...
    this.maybeOpenAndExit(opts)
      .done(function(res) {
        console.log('websocket: created new server')
        ...
        var onBackendRelease = function(releases) {
          var latest = _.reduce(releases, function(acc, release) {
            if (!acc || semver.lt(acc.release, release.release))
              return release
            else
              return acc
          }, null)

          if (latest && semver.gt(latest.release, pkg.version)) {
            console.log('release/backend: update found ' + pkg.version + ' -> '+ latest.release)
            this._requireBackendUpdate = latest
          } else {
            console.log('release/backend: Your up to date!')
          }
        }.bind(this)

        // backend may or may not be emitting a release event
        onBackendRelease(backend.releases())
        backend.on('releases', onBackendRelease)
  ...
  setupServer(opts, frontend) {
    ...
    // setup frontends
    var staticHandlers = {}
    var onFrontendRelease = function(releases) {
      _.each(releases, function(release) {
        release.ensure()
          .done(
            function(release) {
              console.log('adding release', release.release, release.path)
            ...
            function(err) {
              console.log('failed to bind frontend: ', release, err)
    ...
    onFrontendRelease(frontend.releases())
    frontend.on('releases', onFrontendRelease)

    this._http = http.createServer(function(req, res) {
      function err(err) {
        res.statusCode = err.status || 500

        switch (res.status || err.status) {
          case 404:
            fs.readFile('./dist/' + (res.status || err.status) + '.html', 'utf8', function(err, buf) {
              res.end((err || {}).message || buf || "")
            })
            break
      ...
      if (this._requireBackendUpdate) {
        fs.readFile('./dist/update.html', 'utf8', function(error, data) {
          if (error)
          ...
          res.end(markup.up(data, {
              release: this._requireBackendUpdate
          }))

        }.bind(this))
      } else if ('/events' === req.url) {
        initSSE(req, res)
