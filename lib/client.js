import {b64_sha1 as SHA1} from './sha1'
...
export default class Client {
  constructor(port, opts, settings) {
    this._ref  =  opts.ref || this.genref(port.serialNumber || (opts.port || {}).uniqueID)
    ...
    this._uniqueIdentifier = port.serialNumber || (opts.port || {}).uniqueID
    ...
    // DO AUTOCONNECT
    // NEEDS WORK ON SYNCING STATE ON CHANGES
  }
  ...
  genref(stableid, length) {
    length = length || 8
    return SHA1(stableid || Math.random().toString()).substr(0, length)
  }

  setAvailable(port) {
    this._port.path = port.comName
    this._port.port = port
    this._uniqueIdentifier = port.serialNumber || port.uniqueID
